...
wrouter.post("/signup", async (req, res) => {
  ...
  if (adduser.role == "employee") {
    ...
    if (newemployee) {
      return res
        .status(200)
        .send({ msg: "new employee added", data: newemployee });
    } else {
    ...
    if (newcustodian) {
      return res
        .status(200)
        .send({ msg: "new custodian added", data: newcustodian });
    } else {
...
wrouter.post("/login", async (req, res) => {
  ...
  if (req.body.role == "employee") {
    ...
    if (employee) {
      res
        .status(200)
        .send({ msg: "successfull login of employee", data: employee });
    } else {
    ...
    if (custodian) {
      res
        .status(200)
        .send({ msg: "successfull login of custodian", data: custodian });
    } else {
...
wrouter.get("/allcustodians", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ msg: "error in finding all custodians", error: err });
  }
});
